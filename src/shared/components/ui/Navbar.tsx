...
export function Navbar() {
  ...
  ? { background: read ? 'transparent' : 'rgba(53,77,22,0.09)', borderBottom: '1px solid rgba(39,39,42,0.5)' }
  : { background: read ? 'transparent' : 'rgba(53,77,22,0.06)', borderBottom: '1px solid #F0F0F2' }
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6">
    <div className="flex items-center justify-between h-16">
      ...
      <Link to="/" className="flex items-center gap-2.5 flex-shrink-0">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center shadow-glow-sm" style={{ backgroundColor: '#354D16' }}>
          <BookOpen size={16} className="text-white" />
        </div>
        <span className="font-display font-bold text-lg tracking-tight" style={{ color: '#5E6E4E' }}>
  Projeto Tâmaras
</span>
      </Link>
      ...
      <div className="flex items-center gap-1">
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            style={{ color: notifOpen ? '#354D16' : textSecondary }}
            title="Notificações"
          >
            <Bell size={17} />
            {unreadCount > 0 && (
              <span
                ...
                style={{ backgroundColor: '#354D16', borderColor: darkMode ? '#09090B' : '#FFFFFF' }}
              />
            )}
          </button>
          ...
          {notifOpen && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: darkMode ? '1px solid #27272A' : '1px solid #E4E4E7' }}>
                ...
                {unreadCount > 0 && (
                  <button onClick={markAllRead} className="text-xs flex items-center gap-1" style={{ color: '#354D16' }}>
        ...
        {user ? (
          <div className="relative" ref={profileRef}>
            ...
            {profileOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 rounded-xl overflow-hidden z-50 animate-slide-down" style={panelStyle}>
                ...
                <div className="p-1.5">
                  {[
                    ...
                    <Link key={to} to={to} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all hover:bg-green-900/10" style={{ color: textSecondary }} onClick={() => setProfileOpen(false)}>
                      <Icon size={14} />
                      {label}
                    </Link>
                  ))}
                  <button className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all hover:bg-green-900/10 w-full text-left" style={{ color: textSecondary }}>
                    <Settings size={14} />
                    Configurações
                  </button>
                </div>
                <div className="p-1.5" style={{ borderTop: darkMode ? '1px solid #27272A' : '1px solid #E4E4E7' }}>
                  <button onClick={() => { logout(); navigate('/login') }} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-red-500/10 transition-all w-full text-left">
                    <LogOut size={14} />
                    Sair
                  </button>
                </div>
  ...
  {/* Mobile menu */}
  {mobileOpen && (
    <div style={darkMode ? { background: 'rgba(9,9,11,0.97)', borderTop: '1px solid #27272A' } : { background: 'rgba(255,255,255,0.97)', borderTop: '1px solid #E4E4E7' }} className="md:hidden backdrop-blur-md">
      <div className="p-4 space-y-1">
        {navItems.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to} className={({ isActive }) => `nav-link w-full ${isActive ? 'active' : ''}`} onClick={() => setMobileOpen(false)}>
            <Icon size={15} />
            {label}
          </NavLink>
        ))}
...
{searchOpen && (
  <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-start justify-center pt-24 px-4" onClick={() => setSearchOpen(false)}>
    <div className="w-full max-w-2xl rounded-2xl overflow-hidden animate-slide-down" style={panelStyle} onClick={(e) => e.stopPropagation()}>
      <form onSubmit={handleSearch}>
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          <input
            ...
            style={{ background: 'transparent', border: 'none', boxShadow: 'none', padding: '0', color: textPrimary, fontSize: '16px', width: '100%', outline: 'none' }}
          />
